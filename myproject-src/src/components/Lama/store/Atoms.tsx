...
export const paintByMaterialImageState = atom<File | undefined>({
  key: 'paintByMaterialImageState',
  default: undefined,
});
...
export const originFileState = selector({
  key: 'originFileState',
  get: ({ get }) => {
    const app = get(appState);
    return app.originFile;
  },
  set: ({ get, set }, newValue: any) => {
    const app = get(appState);
    set(appState, {
...
export const isProcessingState = selector({
  ...
  get: ({ get }) => {
    const app = get(appState);
    return app.isInteractiveSegRunning || app.isPluginRunning || app.isInpainting;
  },
});
...
export enum BuildingEnvironment {
  城市 = '城市',
  公园 = '公园',
  庭院 = '庭院',
  湖边 = '湖边',
...
export const settingStateDefault: Settings = {
  ...
  showBuildingExterior: false,
  buildingEnvironment: '不选',
...
const localStorageEffect =
  ...
  ({ setSelf, onSet }: any) => {
    ...
    if (savedValue != null) {
      ...
      storageSettings.showPanned = false;
      const restored = _.merge(_.cloneDeep(settingStateDefault), storageSettings);
      setSelf(restored);
    }
  ...
  get: ({ get }) => {
    ...
    switch (settings.model) {
      case AIModel.PAINT_BY_EXAMPLE:
        return settings.paintByExampleSeedFixed ? settings.paintByExampleSeed : -1;
      default:
...
export const isPanned = selector({
  key: 'isPanned',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showPanned;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showPanned: newValue });
  },
});

export const isNoose = selector({
  key: 'isNoose',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showNoose;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showNoose: newValue });
  },
});

export const fileCosUrl = selector({
  key: 'fileCosUrl',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.fileCosUrl;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, fileCosUrl: newValue });
  },
});

export const isEditorPanel = selector({
  key: 'isEditorPanel',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showEditorPanel;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showEditorPanel: newValue });
  },
});

export const isExtend = selector({
  key: 'isExtend',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showExtend;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showExtend: newValue });
  },
});

export const isMaterial = selector({
  key: 'isMaterial',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showMaterial;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showMaterial: newValue });
  },
});

export const isBuildingExterior = selector({
  key: 'isBuildingExterior',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showBuildingExterior;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showBuildingExterior: newValue });
  },
});

export const isRemove = selector({
  key: 'isRemove',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.showRemove;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, showRemove: newValue });
  },
});

export const extendState = selector({
  key: 'extendState',
  get: ({ get }) => {
    const settings = get(settingState);
    return settings.extendState;
  },
  set: ({ get, set }, newValue: any) => {
    const settings = get(settingState);
    set(settingState, { ...settings, extendState: newValue });
  },
});
...
export const runManuallyState = selector({
  ...
  get: ({ get }) => {
    ...
    return settings.runInpaintingManually || isSD || isPaintByExample || isPix2Pix;
  },
});
