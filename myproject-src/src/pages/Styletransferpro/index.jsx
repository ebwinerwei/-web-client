...
const atmosphere_list = [
  { label: t('不限定'), value: '' },
  { label: t('春'), value: '046-春景' },
  { label: t('夏'), value: '047-夏景' },
  { label: t('秋'), value: '048-秋景' },
  { label: t('冬'), value: '049-冬景' },
  { label: t('下雨'), value: '042-雨景' },
  { label: t('雾气'), value: '044-雾天' },
  { label: t('下雪'), value: '043-雪景' },
  { label: t('朝'), value: '037-黎明' },
  { label: t('昼景'), value: '039-日景' },
  { label: t('黄昏'), value: '040-日落' },
  { label: t('夜景'), value: '041-夜景' },
  { label: t('室内照明'), value: '053-内透室内' },
];
const style_qishoushitxt = "002-通用效果图";
...
const Draw = () => {
  ...
  const [lock, set_lock] = useState(0.75);
  const [cnStrength, set_cnStrength] = useState(0.8);
  ...
  const [ckpt, set_ckpt] = useState('010.safetensors');
  // —— 尺寸读取用 ——
  // .right-content 容器
  ...
  useEffect(() => {
    ...
    if (check_history_data?.promptId) {
      ...
      set_lock(check_history_data.lock ?? 0.75);
      set_cnStrength(check_history_data.cnStrength ?? 0.8);
      set_ckpt(check_history_data.ckpt ?? '010.safetensors');
      set_atmosphere(check_history_data.atmosphere ?? '');
  ...
  const clear = useInterval(async () => {
    ...
    if (res.data?.resultImg && res.data?.status == 1 && res.error.errorCode == 0) {
      ...
      setNumber((res.data.queueNum + 1) * 45);
  ...
  const { run: start_task } = useDebounceFn(
    async (num) => {
      ...
      if (!style_img) {
        message.info('設計スタイル参考画像をアップロードしてください');
        return;
      }
      set_has_start(true);
      const params = {
        ...
        ratio: [style_qishoushitxt,atmosphere].filter((v) => Boolean(v.length)).join(','),
        ...
        classifyId1: allClassify[25]?.id,
        classifyWord1: allClassify[25]?.name,
  ...
  return (
    <>
      <Header />
      <div className="styletransferpro">
        <div className="draw-left">
            ...
            <div className="style-item">
              {!style_img ? (
                <Upload
                  ...
                  //   return true;
                  // }}
                  beforeUpload={(file) => validateImageFile(file, t, message)}

                  customRequest={(option) => customRequest(option, 1)}
                >
                  <div className="item-wrapper">
                    <div className="plus">+</div>
                    <div className="plus-label">{t('設計スタイル参考画像をアップロードしてください')}</div>
                  </div>
                </Upload>
              ) : (
          ...
          <div className="angle">
            <div className="space-title"  style={{ width: '50%' }}>{t('モデル設定：')}</div>
            <div className="select">
              <Select
                      defaultValue={"010.safetensors"}
                      value={ckpt}
                      onChange={(e) => set_ckpt(e)}
                      options={[
                        {
                          value: "010.safetensors",
                          label: t('モデルタイプ１（汎用）'),
                        },
                        {
                          value: "xl/blueyuan.safetensors",
                          label: t('モデルタイプ２'),
                        },
                      ]}
              />
          ...
          <div className="angle">
            <div className="space-title" style={{ width: '50%' }}>{t('外観雰囲気：')}</div>
            <div className="select">
              <Select
                value={atmosphere}
            ...
            <div className="left" ref={leftPaneRef}>
              ...
              <div className="bottom-draw">
                <div className="left-wrapper">
                   <Tooltip title={t('ベース画像の形状を維持する程度を設定する。通常はデフォルト値0.8で良いです。')}>
                    <img src={'/imgs/wenhao.png'} className="icon" />
                  </Tooltip>
                  <div className="label">{t('ベース画像形状維持度：')}</div>
                  <div className="slide">
                    <Slider min={0} step={0.05} max={1} onChange={onChange} value={cnStrength} />
                  </div>
                  <Tooltip title={t('参考画像の設計スタイルを適用する程度を設定する。通常はデフォルト値で良いです。')}>
                    <img src={'/imgs/wenhao.png'} className="icon" style={{ marginLeft: 20 }} />
                  </Tooltip>
                  <div className="label">{t('スタイル適用度：')}</div>
                  <div className="select">
                    <Select
                      defaultValue={0.75}
                      style={{
            ...
            <div className="right">
              ...
              <div className="bottom">
                ...
                <div className="title2">
                  {t('ベース画像の形状を維持しながら，参考画像の設計スタイルを機械学習し、ベース画像に適用します')}
                </div>
                <div className="content">
                  <div className="pic-item">
                    <img
                      src={
                        'https://blue-1304000175.cos.ap-tokyo.myqcloud.com/Eva%e7%9b%b8%e5%85%b3%2feva_imgs%2fweb%2fai%e8%af%a6%e6%83%85%e9%a1%b5%e5%8f%b3%e4%be%a7%2f%e9%80%9a%e7%94%a8AI%2fstyletransfer%2fbaseref1.jpg'
                      }
                      className="picture"
                    />
                    <div className="name">{t('ベース画像と設計スタイル参考画像')}</div>
                  </div>
                  <i className="iconfont icon-down-jiantou" />
                  <div className="pic-item">
                    <img
                      src={
                        'https://blue-1304000175.cos.ap-tokyo.myqcloud.com/Eva%e7%9b%b8%e5%85%b3%2feva_imgs%2fweb%2fai%e8%af%a6%e6%83%85%e9%a1%b5%e5%8f%b3%e4%be%a7%2f%e9%80%9a%e7%94%a8AI%2fstyletransfer%2faistyletrans.png'
                      }
                      className="picture"
                    />
                    <div className="name">{t('設計スタイル適用後の画像')}</div>
                  </div>
                </div>
              </div>
            </div>
