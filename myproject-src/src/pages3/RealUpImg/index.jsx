...
const Draw = () => {
  const { allClassify, tickets, get_tickets, set_result_imgs, set_function_page } =
    useModel('global');
  ...
  const { run: start_task } = useDebounceFn(
    async () => {
      ...
      const res = await startTask({
        ...
        classifyId1: allClassify[16]?.id,
        classifyWord1: allClassify[16]?.name,
      });
