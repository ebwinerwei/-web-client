...
ab_path = "/root/autodl-tmp/"

sam_checkpoint = "/root/autodl-tmp/models/sams/sam_vit_h_4b8939.pth"  # "sam_vit_l_0b3195.pth" or "sam_vit_h_4b8939.pth"
...
def transparent_black_to_color(image, color):
  # 将黑色部分调整为透明
...
class PromptServer():
    def __init__(self, loop):
        ...
        @routes.post("/click")
        async def click_images(request):
          form_data = await request.post()
          print('form_data', form_data.get("type"), form_data.get("click_list"), form_data.get("imgUrl"))
          mask_input = None
          segmented_mask = []
          interactive_mask = []

          response = req.get(form_data.get("imgUrl"))
        #   image_data = BytesIO(response.content)
          ...
          img = transparent_black_to_color(res, (255, 204, 0, int(255)))
          return web.json_response({"masks": pil_image_to_base64(img), "message": "Images processed successfully"})
        
        @routes.post("/delImg")
        async def delImg(request):
            form_data = await request.post()
            
            global ab_path
            filename = ab_path+form_data.get("path")
            print('delimg', filename)
            if os.path.exists(filename):
                os.remove(filename)
                return web.json_response({"code": 0, "message": f"文件 {filename} 删除成功！"})
            else:
                return web.json_response({"code": 1, "message": f"文件 {filename} 不存在。"})
        
        @routes.post("/click2")
        async def click_images(request):
          form_data = await request.post()
          print('form_data', form_data.get("type"), form_data.get("click_list"))
          mask_input = None
          segmented_mask = []
          interactive_mask = []
